...
#define Epson_tcp_PORT 4085     //TCP port for Epson
#define Kuka_tcp_PORT 4085	////TCP port for Kuka
#define Epson_tcp_address "192.168.10.42" // Ip address of Epson

/****************End******************/

#define UDP_SERVER_PORT 11111	//udp server port of TSN taker
...
int tcp_client_init(){        
	...
	/*clientSocket*/
	clientSocket = socket(AF_INET, SOCK_STREAM, 0); /*int socket\A3\A8int domain , int type , int protocol)*/
	if(clientSocket == -1)
	{
...
void CMD_handle(int fd)                     // Junhui 0929         
{
	...
	while(1)
	{
		...
		{ 	
			...
			pthread_mutex_unlock(&mutex[0]);//\BD\E2\CB\F8
